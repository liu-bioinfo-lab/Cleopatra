...
def load_loops():
    ...
    for cell in LOOPS:
        all_loops[cell] = {}
        for line in open(LOOPS[cell]):
            if line.startswith('BIN1_CHR'):
                continue
            lst = line.strip().split()
            ch, p1, p2 = 'chr' + lst[0], (int(lst[1]) + int(lst[2])) // 2, (int(lst[4]) + int(lst[5])) // 2
            if p1 > p2:
                p1, p2 = p2, p1
            if ch not in all_loops[cell]:
                all_loops[cell][ch] = []
            all_loops[cell][ch].append((p1, p2))
    for cell in all_loops:
        for ch in all_loops[cell]:
            all_loops[cell][ch] = sorted(all_loops[cell][ch])
    return all_loops


def BiSearch(lst, p1, p2):
    """
    Binary search (for fast identifying all loops in a training region)
    """
    is_tuple = False
    ...
    while ed - st > 1:
        mid = (st + ed) // 2
        val = lst[mid][0] if is_tuple else lst[mid]
        if p1 >= val:
            st = mid
        else:
            ed = mid
...
def load_weights(
    cell, ch, st, ed, res,
    loops=False, radius=10, loop_list={}, loop_weight=2.5
):
    # print(cell, ch, st, ed)
    size = (ed - st) // res
    ...
    if loops:
        if cell in loop_list and ch in loop_list[cell]:
            lst = loop_list[cell][ch]
            pp1, pp2 = BiSearch(lst, st, ed)
            for i in range(pp1, pp2 + 1):
                x1, y1 = (lst[i][0] - st) // res, (lst[i][1] - st) // res
                if x1 - radius >= 0 and y1 - radius >= 0 and x1 + radius < size and y1 + radius < size:
                    # print(' loop:', lst[i])
                    weight[x1 - radius:x1 + radius + 1, y1 - radius:y1 + radius + 1] *= loop_weight
                    weight[y1 - radius:y1 + radius + 1, x1 - radius:x1 + radius + 1] *= loop_weight
...
def training_data_generator(
    ...
    loop = micro_cell_lines[0] in LOOPS
    ...
    for _epoch in range(n_epoches):
        ...
        for _batch in range(len(idx) // batch_size):
            ...
            for i in range(batch_size):
                ...
                weights[i, :, :] = load_weights(
                    cell, ch, pos, pos + max_range, res=resolution,
                    loops=loop, loop_list=loop_list,
                )
